...
- **Buttons** — canonical global system in `styles.css`, compose with `.btn-base`: `.btn-primary` (brand-green gradient fill, e.g. "Get funded") and `.btn-secondary` (green outline, fills on hover, e.g. "Fund Projects"). Brand greens are **fixed hex (identical in light + dark)** — do NOT make buttons theme-variable. Use these everywhere; don't hand-roll `bg-accent text-white rounded-*` buttons. Exceptions left intentionally: input-addon caps (settings "+" buttons, share-modal Copy) and tab/filter toggle states.
